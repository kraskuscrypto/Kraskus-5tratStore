(() => {
    ...
    function setConnectionCopyState(truth) {
        const label = findExactText(document, "USERNAME / WORKER");
        if (!label) return;
        const valueEl = label.nextElementSibling;
        if (valueEl) {
            valueEl.textContent = truth.payoutConfigured && truth.payoutAddress
                ? truth.payoutAddress + "." + workerName()
                : "Configure payout address first";
        }

        const row = ancestorContaining(label, ["USERNAME / WORKER", "Copy"], 5);
        const copyButton = row
            ? Array.from(row.querySelectorAll("button")).find(button => button.textContent.trim() === "Copy")
            : null;
        if (copyButton) {
            copyButton.disabled = !truth.payoutConfigured;
            copyButton.setAttribute("aria-disabled", truth.payoutConfigured ? "false" : "true");
        }
    ...
    function connectionStateMessage(truth) {
        if (truth.readyToMine) {
            return "Ready to mine — node, payout, Stratum, and mining engine are ready.";
        }
        const reasons = [];
        ...
        return reasons.length
            ? "Not ready to mine — " + reasons.join(" and ") + "."
            : "Not ready to mine — waiting for appliance readiness.";
    }
